...
class MyToken
{
    ...
    public function checkToken(string $userToken, string $client_type, string $device_id)
    {
        ...
        if(!$payload || !$header){
            Log::write_log('解析不到数据', '/logs/token');
            return false;
        }
...
//            Log::write_log('缓存的token和上传的token不一致', '/logs/token');
//            return false;
//        }
        //如果缓存不存在或did不一致，或过期返回登录过期
        if($device_id !== $header['did'] || $payload['exp'] < time()){
            Log::write_log('如果缓存不存在或did不一致，或过期返回登录过期', '/logs/token');
            //Redis::hdel($this->redis_token_key, $key);
            return false;
        }
        $uid = $payload['user_id'] - INVITE_PREFIX;//推荐码减推荐码前缀得到uid
        $rst_data = [
            'uid' => $uid,
